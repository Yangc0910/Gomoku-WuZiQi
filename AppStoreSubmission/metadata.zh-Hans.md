# 技能五子棋 App Store 元数据草稿

## App 信息

- App 名称：技能五子棋：离线对战
- 副标题：双人切磋与三档人机挑战
- 类别建议：Games / Board
- 年龄分级建议：4+
- 价格建议：免费
- Bundle ID：`com.chengyang.SkillGomoku`
- 版本号：`2.0`
- 构建号：`6`

## 宣传文本

离线五子棋现已支持三档人机挑战，也可体验本机双人、标准技能和高阶技能模式；无需账号或网络。

## 此版本的新增内容

技能五子棋 2.0 新增离线人机单机模式：

- 新增轻松、标准、困难三档本地 AI
- 支持玩家选择先手或后手，电脑会自动完成自己的回合
- 新增 AI 思考状态、整回合悔棋和人机对局自动保存恢复
- 人机战绩只计入真实玩家档案
- AI 完全在设备上运行，不需要登录或联网

## 描述

技能五子棋是一款支持离线人机挑战与本机双人对弈的原生 iOS 五子棋应用。当前版本提供清晰的 15x15 棋盘、经典五子棋、三档电脑对手、标准技能模式、高阶技能模式、玩家档案、本地头像、自动保存恢复、撤销、胜负结算与战绩统计。

可以随时独自挑战电脑，也适合两位玩家在同一台 iPhone 或 iPad 上快速开局。应用不需要账号，不联网，不上传头像或对局数据。所有玩家资料、头像、对局和 AI 计算都留在本机。

当前版本功能：

- 经典五子棋对局
- 轻松、标准、困难三档离线人机对战
- 自选先手或后手，电脑自动落子
- 标准技能五子棋，内置五项固定技能
- 高阶技能五子棋，开局选择三项技能组合
- 本机双人玩家选择
- 玩家档案与头像设置
- 照片选择与相机拍照入口
- 模拟器或无相机设备上的相机不可用提示
- 对局自动保存与首页继续对局
- 双人模式撤销上一步，人机模式撤销一整轮
- 获胜、平局与回合数结算
- 本地胜负平统计
- iPhone 与 iPad 自适应布局

## 关键词

五子棋,技能五子棋,人机对战,双人游戏,棋盘游戏,本机对战,策略,休闲,连珠,离线游戏

## 截图映射

以下现有截图来自 1.1 界面，可继续展示双人和技能玩法；提交 2.0 前应补充至少一张人机设置页和一张 AI 对局页截图。

- App Icon 预览：`AppStoreAssets/app-icon/app-icon-1024.png`
- iPhone 6.5 英寸：
  - `AppStoreAssets/iphone-6.5/01-core-gameplay.png`
  - `AppStoreAssets/iphone-6.5/02-skill-mode.png`
  - `AppStoreAssets/iphone-6.5/03-match-setup.png`
- iPhone 6.9 英寸：
  - `AppStoreAssets/iphone-6.9/01-core-gameplay.png`
  - `AppStoreAssets/iphone-6.9/02-skill-mode.png`
  - `AppStoreAssets/iphone-6.9/03-match-setup.png`
- iPad 13 英寸：
  - `AppStoreAssets/ipad-13/01-core-gameplay.png`
  - `AppStoreAssets/ipad-13/02-skill-mode.png`
  - `AppStoreAssets/ipad-13/03-match-setup.png`

## App Privacy 建议

- Data Collection：No, we do not collect data from this app.
- Tracking：No.
- 说明：玩家姓名、头像、对局记录和战绩仅保存在本机，不上传、不共享、不用于追踪。

## Export Compliance 建议

- App 不包含自定义加密。
- App 当前不联网，不使用账号系统，不调用后端服务。

## Support URL / Privacy Policy URL

需要发布为公开 HTTPS 页面后填入 App Store Connect。可使用本目录中的：

- `privacy-policy.zh-Hans.md`
- `support.zh-Hans.md`
