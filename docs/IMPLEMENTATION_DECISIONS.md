...
- 所有技能统一生成 `SkillPresentation`，包含发动方、受影响位置、位移路径与结果说明。
- 技能结算分为发动预告、棋盘状态提交和结果停留三段；完整反馈持续 2.2 秒，状态在约 0.38 秒时提交。
- 动效期间禁用棋盘、技能和悔棋输入，待反馈完成后才启动 AI 或移交双人对局回合。
- 九项技能分别使用风沙、清扫、恢复光柱、阵营旋转、震波、位移轨迹、封印、复活和护盾视觉语言。
- 辅助功能的“减少动态效果”开启时，保留技能名称和结果信息，将棋盘动画简化为静态关键帧。
- AI 搜索完成后额外保留约 1 秒思考状态，避免落子或技能在玩家回合后立即闪现。
- 开局前的对手、档案、先后手、难度与技能选项统一使用描边、勾选状态和轻微按压反馈；人机档案直接展示头像卡片，减少隐藏在菜单中的选择。
