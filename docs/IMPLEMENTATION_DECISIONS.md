...
- 棋盘将整个可见区域划分为 15 × 15 命中单元，四周半格也会归入最外侧交叉点；轻点使用起始触点消除松手漂移，按住拖动则实时预览并在最终交叉点落子。
- 点击已占位置会显示短暂非法反馈。
...
- 长按放大镜未在本轮实现；当前提供全棋盘交叉点吸附、轻点防漂移、按住滑动校准、非法落点反馈和最后一步标识。
- 结算页先作为结果 sheet 展示；后续可扩展为更完整的再来一局、换模式和分享入口。

## 2.0 人机单机模式

- `singlePlayer` 原始值继续承载旧版经典人机存档；新增技能人机局复用 `standardSkills` 与 `advancedSkills` 规则值，通过 `computerOpponent` 区分对手类型。
- `ComputerOpponent` 随 `GameState` 编码，记录电脑阵营与难度；旧存档缺少该可选字段时仍按双人模式读取。
- AI 采用“立即胜负检查 + 棋形启发式 + 困难档极小化搜索”，仅评估已有棋子附近的候选点。
- AI 输出完整 `GameAction`，可在标准与高阶模式中选择合法技能、目标或普通落子。
- 入口信息架构先按“离线人机 / 本机双人”分类，再选择经典、标准技能或高阶技能规则。
- AI 搜索在后台任务执行，状态提交与 SwiftData 保存仍由 `@MainActor` 的 `MatchViewModel` 管理。
- 电脑使用固定参与者 ID，不写入玩家档案；人机赛果只更新真实玩家的统计。
- 人机悔棋按一整轮处理，避免撤销后停留在电脑回合中间状态。
